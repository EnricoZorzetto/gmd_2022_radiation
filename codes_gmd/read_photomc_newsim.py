...
datadir_diffs = os.path.join(result_output_folder, 'simul_differences')
datadir_models = os.path.join(result_output_folder, 'trained_models')
datadir_crossval = os.path.join(result_output_folder, 'crossval_models')
datadir_gof_metrics = os.path.join(result_output_folder, 'gof_metrics')
    ...
    for id, mydomain in enumerate(DOMAINS):
        for ibu, mybuffer in enumerate(CROP_BUFFERS):
            for ibl, myaveblock in enumerate(AVEBLOCKS):
                ...
                simul_name = 'output_cluster_PP3D_{}_{}'.format(mydomain, adir_prod)
                ...
                if plot_original_fields and ibu == 0 and myaveblock in AVEBLOCKS_TO_PLOT: # plot fields only in this case
...
for id, mydomain in enumerate(DOMAINS):
    for ibu, mybuffer in enumerate(CROP_BUFFERS):
        for ibl, myaveblock in enumerate(AVEBLOCKS):
            ...
            if not test_only: # we are actually training the models here::
                test_savedir = None
                ...
                if not test_single_aveblock: # do not use same aveblock for model to be tested
                    prefit_models_aveblock = myaveblock
                ...
                model_savedir = os.path.join(datadir_models,
                        'domain_{}_buffer_{}'.format(prefit_models_domain, mybuffer),
                        'models_ave_{}'.format(prefit_models_aveblock) )
                ...
                test_savedir = os.path.join(datadir_crossval,
                        ...
                        prefit_models_domain, prefit_models_adir, prefit_models_aveblock,
                        mydomain, myaveblock ))
            ...
            # print("TEST:")
            # print(np.corrcoef(np.ravel(ds_sin['elen'].values), np.ravel(ds_sin['svfn'].values)))
            # exit()

            # use the variables not normalized by cosz for perdiction
            if not normalize_Vd_and_Ct:
            ...
            if id==0 and ibu==0 and ibl==0:
                model_metrics = dem.init_metrics_dataset(
                        DOMAINS, AVEBLOCKS, CROP_BUFFERS, ADIRs,
            ...
            for ic, mycosz in enumerate(COSZs):
                for ia, myadir in enumerate(ADIRs):
                    ...
                    dfflux = pd.DataFrame({
                        mylab:   ds_sin[mylab][:,   ic, ia].values for mylab in LABELS})
                    # create dataframe with predictors for testing or training models
                    dfvars = pd.DataFrame({
                        mypred: ds_sin[mypred][:].values for mypred in PREDICTORS_TO_USE
                        if mypred not in ['sian']})
                    if 'sian' in PREDICTORS_TO_USE:
                        dfvars['sian'] = ds_sin['sian'][:,ic].values
                    dfvars = dfvars[PREDICTORS_TO_USE] # re-order columns
                    ...
                    if not test_only:
                        ...
                        if remove_null_obs:
                            ...
                            dftot1 = dftot
                        if resample_data and resample_data_size < dftot1.shape[0]:
                            dftot2 = dftot1.sample(n = resample_data_size, random_state=0)
                        else:
                            dftot2 = dftot1
                        dfvars = dftot2[PREDICTORS_TO_USE]
                        dfflux = dftot2[LABELS]


                    for ines, rf_n_estim in enumerate(RF_N_ESTIMATORS):
                        for imtd, rf_max_dep in enumerate(RF_MAX_DEPTH):
                            ...
                            for ilab, mylab in enumerate(LABELS):
                                for mymet in GOF_METRICS:
                                    for mymod in MODELS:
                    ...
                    model_metrics.attrs['test_only'] = int(test_only)
                    if test_only:
                        model_metrics.attrs['cv_training_domain'] = prefit_models_domain
...
if test_only: # save metric file tracking domain and aveblock of the model tested
    ...
    file_res = os.path.join(datadir_gof_data,
            ...
            prefit_models_domain, prefit_models_adir, save_prefit_models_aveblock))
else:
    file_res = os.path.join(datadir_gof_data,
...
MODELS_TO_PLOT = ['MLR', 'RFR']
print("plotting goodness of fit measures...")
dem.plot_gof_measures(file_res, datadir_gof_plots,  METRICS_TO_PLOT = GOF_METRICS, MODELS_TO_PLOT=MODELS_TO_PLOT)
...
if len(RF_N_ESTIMATORS) > 1: # explore RF performance varying hyper param
...
AVELABELS_TO_HIST = [0.1, 0.5, 3, 5, 10]
# AVEBLOCKS_TO_PLOT =   [55, 110] # must be a subset of AVEBLOCKS
