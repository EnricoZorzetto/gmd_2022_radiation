...
# simul_name = 'output_cluster_PP3D_Nepal_run1_a35'
...
# if True, fit the models
# if False, use models already fit stored here
do_fit_models = True

prefit_model_name = 'EastAlps'
prefit_model_folder = os.path.join( simul_folder, 'models_{}'.format(prefit_model_name))

if do_fit_models:
    prefit_model_name = domain_name
    prefit_model_folder = datadirm
...
read_original_output = True
# list of output files to extract
SIMUL_NAMES = [
    'output_cluster_PP3D_EastAlps_run1_a35',
    'output_cluster_PP3D_EastAlps_run1_a17',
    'output_cluster_PP3D_Peru_run1_a35'
]
...
if read_original_output:
    for ib in range(naveblocks):
        ...
        for ins in range(len(SIMUL_NAMES)):
            simul_name = SIMUL_NAMES[ins]
...
for ib in range(naveblocks):
    ...
    mlr_savedir = os.path.join(prefit_model_folder,
    ...
    for ic, mycosz in enumerate(COSZs):
        ...
        for il, label in enumerate(LABELS):
            ...
            rf_savename = 'rf_model_{}_cosz_{}_adir_{}'.format(
            ...
            if do_fit_models:
                ...
                with open(os.path.join(mlr_savedir, '{}.pickle'.format(rf_savename)),
                          'wb') as pkrf:
                    pickle.dump(regressor, pkrf)


            with open(os.path.join(mlr_savedir,
                        '{}.pickle'.format(rf_savename)), 'rb') as pkrf:
                # to read the saved model, do the following:
                rf_model = pickle.load(pkrf)


            yrf_pred = rf_model.predict(Xrf_test)
            yrf_pred_testalbedo = rf_model.predict(Xrf_test_testalbedo)
            ...
            if do_fit_models:
                ...
                mlr_model0 = dem.train_multlr(dfred, label=label,
                                     train_frac = train_frac,
                                     plot=False,
                                     save=True,
                                     savedir=mlr_savedir,
