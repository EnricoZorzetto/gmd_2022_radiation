...
# resdir = os.path.join(datadir, 'output_GFDL_singletimestep')
resdir = os.path.join(datadir, 'output_GFDL_singletimestep_ENFRAC_AF')
...
# use a single albedo value for this analysis
adir = metadata['ADIR'][0]
adif = metadata['ADIF'][0]
...
# ground valyues only
if metadata['aerosol']:
    dir_inc_am4 = ds['rsdcs_direct'][0,:,-1,indy, indx]
    tot_inc_am4 = ds['rsdcs'][0,:,-1,indy, indx]
    dif_inc_am4 = tot_inc_am4 - dir_inc_am4
else:
    dir_inc_am4 = ds['rsdcsaf_direct'][0,:,-1,indy, indx]
    tot_inc_am4 = ds['rsdcsaf'][0,:,-1,indy, indx]
    dif_inc_am4 = tot_inc_am4 - dir_inc_am4
...
for icz in range(ncosz):
    for ifr in range(nfreq):
        ...
        # adir = 0.34
        # adif = 0.32
        dir_inc_rmc = RES_PP['EDIR']/(1-adir)
        dif_inc_rmc = (RES_PP['EDIF'] + RES_PP['ECOUP'])/(1-adif)

        dir_inc_RMC.append( dir_inc_rmc )
        dif_inc_RMC.append( dif_inc_rmc )
        dir_inc_AM4.append( dir_inc_am4[myfreq])
        dif_inc_AM4.append( dif_inc_am4[myfreq])
...
axes[0].plot(dir_inc_AM4, '-o', label = 'AM4')
axes[0].plot(dir_inc_RMC, '-*', label = 'RMC')
axes[1].plot(dif_inc_AM4, '-o', label = 'AM4')
axes[1].plot(dif_inc_RMC, '-*', label = 'RMC')
axes[1].set_xlabel(r'gpoints [2 bands: 12850 - 16000 - 22650 $\mathrm{cm^-1}$]')
axes[0].set_ylabel(r'Flux [$\mathrm{W m^{-2}}$]')
axes[1].set_ylabel(r'Flux [$\mathrm{W m^{-2}}$]')
axes[0].set_title('Shortwave direct down at the surface')
axes[1].set_title('Shortwave diffuse down at the surface')
axes[0].legend()
axes[1].legend()
plt.savefig( os.path.join(outfigdir,
        'test_AM4_aer_{}.png'.format(metadata['aerosol'])), dpi = 300)
plt.show()
