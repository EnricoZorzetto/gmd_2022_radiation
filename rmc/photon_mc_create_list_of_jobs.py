...
def init_simulation_wrapper(metadata):
    ...
    # PREPROCESS LAND DEM HERE - save pickled map in preprocessed_dems
    ...
    fl.close()
    df.to_csv( os.path.join(outdir, 'list_jobs_params.csv'), index=False)
    ...
    # df_torun.to_csv( os.path.join(outdir, 'list_jobs_params_to_rerun.csv'), index=False)
